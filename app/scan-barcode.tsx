...
export default function HomeScreen() {
  ...
  const autoOpenedBarcodeMode = useRef(false);
                  ...
                  text: `You are a resale expert. I am showing you ${images.length} photo(s) of the same item. Analyze it like a reseller preparing to list it online. For pricing, estimate a realistic low_price and high_price by checking the item against current eBay and Facebook Marketplace listings for identical or very similar items whenever that market context is available to you. Favor recent sold or active comparable listings, adjust for condition, brand, completeness, and visible wear, and make sure the suggested price range follows the real market rather than a generic guess. Also provide a floor_price, meaning the lowest reasonable price a seller should accept before walking away during negotiation. Then respond ONLY with raw JSON, no markdown:
{
  ...
  const analyzeBarcode = async (barcode: string) => {
    const normalizedBarcode = sanitizeBarcodeValue(barcode);
    if (!normalizedBarcode) {
      Alert.alert("Invalid barcode", "Try scanning the barcode again.");
      return;
    }
    ...
    try {
      const product = await lookupBarcodeProduct(normalizedBarcode);
...

{
  ...
  "listing_description": "3-4 sentence listing description"
      ...
      Alert.alert(
        ...
        error instanceof Error
          ? error.message
          : "We couldn't pull product info from that barcode. Try scanning again with the code centered and well lit.",
        [
          { text: "OK", style: "cancel" },
          {
            text: "Use Barcode Anyway",
            onPress: () => {
              void continueWithBarcodeFallback(normalizedBarcode);
            },
          },
          {
            text: "Scan Again",
  ...
  const continueWithBarcodeFallback = async (barcode: string) => {
    setLoading(true);
    setCurrentItemId(null);

    try {
      const parsed = await buildFallbackBarcodeDraft(barcode);
      setImages([]);
      ...
      Alert.alert(
        ...
        "We couldn't build a fallback draft from that barcode. Try scanning again or switch to photo capture.",
      );
    }
  ...
  const openBarcodeScanner = async () => {
    if (!activePallet) {
      Alert.alert(
        ...
        "Pick or create a pallet before scanning barcodes.",
      );
      return;
    }
  ...
  useEffect(() => {
    if (autoOpenedBarcodeMode.current || !activePallet) {
      return;
    }

    autoOpenedBarcodeMode.current = true;
    void (async () => {
      const permission = cameraPermission?.granted
        ? cameraPermission
        : await requestCameraPermission();
  ...
  }, [activePallet, cameraPermission, requestCameraPermission]);
  ...
  return (
    ...
    >
      ...
      >
        <View style={styles.captureHeader}>
          <View style={styles.captureHeaderCopy}>
            ...
            <Text style={styles.subtitle}>
              Scan a barcode and build a draft from product data.
            </Text>
          </View>
          ...
          <View style={styles.palletHeader}>
            <View>
              ...
              <Text style={styles.palletMeta}>
                {activePallet
                  ? "Scans and saved drafts will go into this pallet."
                  : "Choose a pallet on Home or in the Pallets tab before scanning."}
              </Text>
            </View>
          </View>
        </View>
        ...
        >
          <Text style={styles.barcodeBtnText}>Open Scanner</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  btnText: { color: "#111827", fontWeight: "600", fontSize: 15 },
  barcodeBtn: {
