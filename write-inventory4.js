...
export default function InventoryScreen() {
  const items = (global as any).inventory ?? [];
  ...
  const removeItem = (id: number) => {
    Alert.alert("Remove Item", "Are you sure?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: () => {
          (global as any).inventory = (global as any).inventory.filter((item: any) => item.id !== id);
        },
      },
    ]);
  };
  ...
  const emailInventory = async () => {
    ...
    try {
      ...
      if (!isAvailable) {
        Alert.alert("No mail app", "Please set up a mail account on your phone.");
        return;
      }
...
const styles = StyleSheet.create({
  ...
  exportBtn: { flex: 1, backgroundColor: "#111", padding: 12, borderRadius: 8, alignItems: "center" },
  exportBtnText: { color: "#fff", fontWeight: "500", fontSize: 14 },
  emailBtn: { flex: 1, backgroundColor: "#2d6a4f", padding: 12, borderRadius: 8, alignItems: "center" },
